...
int main(int argc, char **argv) {
	...
	/*flags = checkError(fcntl(fd_server, F_GETFL),
	 "Could not get the flags on the TCP socket");
	 checkError(fcntl(fd_server, F_SETFL, flags | O_NONBLOCK),
	 "Could not the TCP socket to be non-blocking");*/
	...
	while (1) {
		...
		/* Keep keepalive on or off*/
		/*setsockopt(fd_client, SOL_SOCKET, SO_KEEPALIVE, &keepalive_enabled,
		 sizeof(keepalive_enabled));
		 if (keepalive_enabled) {
		 setsockopt(fd_client, IPPROTO_TCP, TCP_KEEPIDLE, &keepalive_time,
		 sizeof(keepalive_time));
		 setsockopt(fd_client, IPPROTO_TCP, TCP_KEEPCNT, &keepalive_count,
		 sizeof keepalive_count);
		 setsockopt(fd_client, IPPROTO_TCP, TCP_KEEPINTVL,
		...
		if (fd_client == -1) {
			...
			if (pid == 0) {
				/*setsockopt(fd_client, SOL_SOCKET, SO_RCVTIMEO,
